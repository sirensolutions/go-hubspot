...
func TestOwnerServiceOp_Create(t *testing.T) {
	owner := &hubspot.Owner{
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				ownerPath: hubspot.ExportDealBasePath,
				client: hubspot.NewMockClient(&hubspot.MockConfig{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := tt.fields.client.CRM.Owner.Create(tt.args.owner)
...
func TestOwnerServiceOp_Update(t *testing.T) {
	owner := &hubspot.Owner{
		Email:       hubspot.NewString("hubspot@example.com"),
		FirstName:   hubspot.NewString("Bryan"),
		LastName:    hubspot.NewString("Cooper"),
	...
	tests := []struct {
		...
		{
			name: "Successfully update a owner",
			fields: fields{
				ownerPath: hubspot.ExportOwnerBasePath,
				client: hubspot.NewMockClient(&hubspot.MockConfig{
					Status: http.StatusCreated,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := tt.fields.client.CRM.Owner.Update(tt.args.ownerID, tt.args.owner)
			if !reflect.DeepEqual(tt.wantErr, err) {
	...
	}{
		{
			...
			want: &hubspot.ResponseResource{
				...
				Properties: &hubspot.Owner{
					...
					HsObjectID:                                  hubspot.NewString("owner001"),
					HsPersona:                                   nil,
...
func TestOwnerServiceOp_Delete(t *testing.T) {
	type fields struct {
		ownerPath string
		client    *hubspot.Client
	}
	...
	tests := []struct {
		...
		{
			name: "Successfully delete a owner",
			fields: fields{
				ownerPath: hubspot.ExportOwnerBasePath,
				client: hubspot.NewMockClient(&hubspot.MockConfig{
					Status: http.StatusCreated,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			err := tt.fields.client.CRM.Owner.Delete(tt.args.ownerID)
			if !reflect.DeepEqual(tt.wantErr, err) {
...
func TestOwnerServiceOp_AssociateAnotherObj(t *testing.T) {
	type fields struct {
		ownerPath string
		client    *hubspot.Client
	}
	...
	}{
		{
			...
			args: args{
				...
				conf: &hubspot.AssociationConfig{
					...
					ToObjectID: "deal001",
					Type:       hubspot.AssociationTypeOwnerToDeal,
				},
			},
			want: &hubspot.ResponseResource{
				ID:       "owner001",
				...
				Associations: &hubspot.Associations{
					Deals: struct {
						...
						Results: []hubspot.AssociationResult{
							{ID: "deal001", Type: string(hubspot.AssociationTypeOwnerToDeal)},
						},
					},
				},
				Properties: &hubspot.Owner{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := tt.fields.client.CRM.Owner.AssociateAnotherObj(tt.args.ownerID, tt.args.conf)
			if !reflect.DeepEqual(tt.wantErr, err) {
